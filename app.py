...
sec_serverURL = "http://127.0.0.3:8000"
...
async def gradient():
    async with aiohttp.ClientSession(serverURL) as s:
        ...
        async with s.get("/login/checkstatus/", json=({
            ...
            rsp2 = eval(await rsp2.text())
        return render_template('gradient.html', username=session.get('username'),
                               email=session.get('email'), admin=rsp2['data'])


@app.route('/gradient/data/', methods=['GET', 'POST'])
async def gradientdata():
    async with aiohttp.ClientSession(serverURL) as s:
        if session.get('email') is not None and session.get('username') is not None:
            async with s.post("/finduser/", json=({
                'operation_key': generate_password_hash(o_key),
        ...
        async with s.get("/login/checkstatus/", json=({
            ...
            rsp2 = eval(await rsp2.text())
            async with s.get("/gradient/date/", json=({
                'operation_key': generate_password_hash(o_key)
            })) as rsp:
                rsp = eval(await rsp.text())
                async with aiohttp.ClientSession(sec_serverURL) as s1:
                    async with s1.get("/gradient/print/", json=({
                        'operation_key': generate_password_hash(o_key),
                        'data': 'start'
                    })) as rsp1:
                        rsp1 = eval(await rsp1.text())
                        if rsp['data'] != 0:
                            time = datetime.datetime.timestamp(datetime.datetime.now())
                            return render_template('gradient_data.html', data=rsp1, username=session.get('username'),
                                                   email=session.get('email'), rsp=rsp, time=int(time),
                                                   admin=rsp2['data'])

            return redirect(url_for('gradient'))
...
async def gradientstart():
    async with aiohttp.ClientSession(serverURL) as s:
        ...
        async with s.get("/gradient/date/", json=({
            ...
            if time - rsp['date'] >= 30:
                async with s.get("/gradient/start/", json=({
                    'operation_key': generate_password_hash(o_key),
                    'date': int(time)
                })):
                    pass
                async with aiohttp.ClientSession(sec_serverURL) as s1:
                    async with s1.get("/gradient/start/", json=({
                        'operation_key': generate_password_hash(o_key),
                        'data': 'start'
                    })):
                        pass
        return redirect(url_for('gradientdata'))
